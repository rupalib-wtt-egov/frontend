...
class FieldList extends Component {
  ...
  componentDidMount = () => {
          ...
          if (card.subCards) {
            card.subCards.forEach(subCard => {
              ...
              if (
                ...
                fieldDocumentsUploadRedux[index] = {
    ...
    prepareFinalObject("fieldDocumentsUploadRedux", fieldDocumentsUploadRedux);
  };

  prepareDocumentsInEmployee = async (fieldDocuments, bpaDetails) => {
    let documnts = [];
    if (fieldDocuments) {
      Object.keys(fieldDocuments).forEach(function (key) {
        if (fieldDocuments && fieldDocuments[key]) {
          documnts.push(fieldDocuments[key]);
        }
      });
    }

    prepareFinalObject("fieldDocumentsUploadRedux", {});
  ...
  handleDocument = async (file, fileStoreId) => {
    ...
    const { prepareFinalObject, fieldDocumentsUploadRedux, bpaDetails } = this.props;
    const fileUrl = await getFileUrlFromAPI(fileStoreId);

    let fieldDocuments = {
      ...fieldDocumentsUploadRedux,
      [uploadedDocIndex]: {
        ...fieldDocumentsUploadRedux[uploadedDocIndex],
        documents: [
          {
            fileName: file.name,
            fileStoreId,
    ...
    prepareFinalObject("fieldDocumentsUploadRedux", fieldDocuments);
  
    let isEmployee = process.env.REACT_APP_NAME === "Citizen" ? false : true

    if(isEmployee) {
      this.prepareDocumentsInEmployee(fieldDocuments, bpaDetails);
      }
  ...
  removeDocument = remDocIndex => {
    ...
    prepareFinalObject(
      `fieldDocumentsUploadRedux.${remDocIndex}.documents`,
      undefined
    );
    this.forceUpdate();
  };
  ...
  handleChange = (key, event) => {
    const { fieldDocumentsUploadRedux, prepareFinalObject, bpaDetails } = this.props;   
    let fieldDocuments = {
      ...fieldDocumentsUploadRedux,
      [key]: {
        ...fieldDocumentsUploadRedux[key],
  ...
  getUploadCard = (card, key) => {
    const { classes, fieldDocumentsUploadRedux } = this.props;
    let jsonPath = `fieldDocumentsUploadRedux[${key}].dropDownValues.value`;
    return (
      <Grid container={true}>
        <Grid item={true} xs={2} sm={1} className={classes.iconDiv}>
          {fieldDocumentsUploadRedux[key] && fieldDocumentsUploadRedux[key].documents ? (
...
const mapStateToProps = state => {
  ...
  const fieldDocumentsUploadRedux = get(
  ...
  return { fieldDocumentsUploadRedux, moduleName, bpaDetails };
};
